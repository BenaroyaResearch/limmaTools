...
#' @param counts a matrix or data frame containing the gene expression data. Should have samples in columns and genes in rows. Rownames must contain gene names corresponding to gene names in \code{topGenes}.
...
#' @usage \code{
#' getCountsSigGenes(counts, topGenes,
#'                   p_cut=0.01, fc_cut=log2(1.5),
#'                   p_col="adj.P.Val", fc_col="logFC",
#'                   threshold_col=NULL
#'                   )}
getCountsSigGenes <- function(counts, topGenes,
                              p_cut=0.01, fc_cut=log2(1.5),
                              p_col="adj.P.Val", fc_col="logFC",
                              threshold_col=NULL) {
