...
get_topGenes_combined <- function(topGenes,
                                  topGenes.pairwise,
                                  p_cut=0.01, fc_cut=log2(1.5)) {
  topGenes.combined <-
  ...
  adj.p.values <-
    matrix(data=p.adjust(as.matrix(topGenes.combined[,str_detect(names(topGenes.combined), "P.Value.")]),
                         method="BH"),
           ncol=length(topGenes.pairwise), dimnames=list(rownames(topGenes.combined),
